...
a.append([])
L=5
layer_size=np.array([784, 128, 64,32,10])
w=[]
...
# for iw in range(0,L-1):
#     temp=0.05*np.random.randn(layer_size[iw],layer_size[iw+1])
#     w.append(temp.T)
for iter in range(0,max_epoch):
    ...
    for i in range(0,int(train_size/mini_batch)):
        ...
        delta.append((a[L-1]-y)*d_sigmoid(z[L-1]))#10*100
